	...
	const float *A, const float *B, float *C, int N, int M, int K, float alpha, float beta)
{
	...
	double operation_time = ((double)(end - start)) / CLOCKS_PER_SEC * 1000;
	double operation_time_1_epoch = operation_time / test_iterations;
...
int main()
{
	...
	float alpha = 2.f;
	float beta = 1.f;
	N = M = K = 512; // Reduced size for testing
